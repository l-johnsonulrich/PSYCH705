...
#examine variables 
hist(bern_df$veg)
hist(bern_df$presence)


# Fit Bernoulli logistic regression (GLM; can also be glmm if needed)
m_bern <- glmmTMB(presence ~ veg, family = binomial(link = "logit"), data = bern_df)
summary(m_bern)
#log odd scale, so we use exp() to transform same as with Poisson
exp(0.5219) #1.68
# “For every one-unit increase in vegetation density, the odds of finding a warbler are about 1.6x times higher.”
# 1.68 is an "odds-ratio" 
...
#looks wonky:
hist(resid(m_bern))
plot(predict(m_bern), resid(m_bern))
...
m_bin <- glmmTMB(cbind(hatched, n_eggs - hatched) ~ veg, family = binomial(link="logit"), data = bin_df)
summary(m_bin)
...
# Explore the data
hist(birds$prop_cover)
hist(birds$prop_squeezed)
hist(birds$prop_adj)
boxplot(birds$prop_cover ~ birds$habitat)


# Fit Beta regression
m_beta <- glmmTMB(prop_adj ~ habitat,
...
m_beta2 <- glmmTMB(prop_squeezed ~ habitat,
                  family = beta_family(link = "logit"),
                  data = birds)
...
ggplot(birds, aes(x = habitat, y = prop_squeezed)) +
  ...
  geom_errorbar(data=emmeans(m_beta2, ~ habitat, type="response") %>% as_tibble(), aes(x=habitat, y=response, ymin = lower.CL, ymax = upper.CL), color="red", width=.5)
...
#correlation test
cor.test(movement_wide$C1, movement_wide$C2)

#icc on the raw data
irr::icc(movement_wide %>% dplyr::select(C1,C2))

#similar to ICC in a GLMM
performance::icc(glmmTMB(total_duration~1+(1|rat), data=movement_dur))
...
#kappa for agreemnt
irr::kappa2(counts_wide %>% filter(behavior=="bipedal") %>% dplyr::select(C1, C2))
irr::kappa2(counts_wide %>% filter(behavior=="startle") %>% dplyr::select(C1, C2))

#correlation
cor.test(~C1 + C2, data=counts_wide %>% filter(behavior=="bipedal"))
cor.test(~C1 + C2, data=counts_wide %>% filter(behavior=="startle"))

#icc
irr::icc(counts_wide %>% filter(behavior=="bipedal") %>% dplyr::select(C1,C2))
irr::icc(counts_wide %>% filter(behavior=="startle") %>% dplyr::select(C1,C2))
...
#correlation
cor.test(~C1 + C2, data=latency_wide)

#icc
irr::icc(latency_wide %>% dplyr::select(C1,C2))
...
startles <- raw_data %>% filter(behavior=="startle")
...
#kappa for agreement
irr::kappa2(matches %>% dplyr::select(C1, C2))
