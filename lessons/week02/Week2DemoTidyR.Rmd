...
#create a boxplot 
data2 %>% 
  ggplot(aes(x=MonthName, y = Solar.R)) + 
  geom_boxplot(fill="gold", color="black")+ 
  theme_bw()

#create a scatterplot
data2 %>%
  ggplot(aes(x=Wind, y=Solar.R)) + geom_point() +
  geom_smooth(method="lm") +
  theme_bw()

#create a line plot 
data2 %>%
  ggplot(aes(x=Date, y = Temp, color=Temp)) + geom_path() + 
  scale_color_gradient(low = "blue", high = "red") +
  theme_bw() +
  labs(title= "Temperatures in Summer 1973", x = "Date", y = "Temperature", color = "°F")

#create a barplot with summary data 
data2 %>% 
  group_by(MonthName) %>% 
  summarise(meanTemp = mean(Temp, na.rm=TRUE), sdTemp = sd(Temp, na.rm=TRUE)) %>%
  ggplot(aes(x=MonthName, y=meanTemp)) + 
