...
# Examine the data 
hist(dat$vigilance_time)
hist(dat$group_size)
hist(as.numeric(dat$predator))

plot(dat$group_size, dat$vigilance_time)
...
v1 <- glmmTMB(vigilance_time ~ group_size, data = dat)
summary(v1)

r2(v1)
simulateResiduals(v1, plot = TRUE)

#Next, let's also add the effect of predator
v2 <- glmmTMB(vigilance_time ~ group_size + predator, data=dat)
summary(v2)

r2(v2)
simulateResiduals(v2, plot=T)
check_collinearity(v2)

#Next, let's check for an interaction 
v3 <- glmmTMB(vigilance_time ~ group_size + predator + group_size*predator, data=dat)
summary(v3)

r2(v3)
simulateResiduals(v3, plot=T)
check_collinearity(v3) #this will also show collinearity when interaction effects involved (use results from v2)
...
estimate_slopes(v3, trend="group_size")
emmeans(v3, ~predator)
emmeans(v3, pairwise ~ predator)
estimate_slopes(v3, trend="group_size", by="predator")

#create predicted data for plotting 
emm <- emmeans(v3, ~group_size*predator, at=list(group_size=1:20)) %>% as_tibble()
...
hist(dat$A)
hist(dat$B)
barplot(table(dat$call_type))
plot(dat$A, dat$B) #big cloud! 


# First, fit a simple model. 

c1 <- glmmTMB(A ~ B, data=dat)
summary(c1)
#weak correlation 

#look at model fit 
r2(c1) # a lot of variation unexplained! 
simulateResiduals(c1, plot=T)

#add the moderator
c2 <- glmmTMB(A ~ B + B:call_type, data=dat)
summary(c2)
#how to interpret?! 
...
#check model fit
r2(c2)
simulateResiduals(c2, plot=T)
check_collinearity(c2) #interaction! 

c3 <- glmmTMB(A ~ B + call_type, data=dat) #this is a non-biologically plausible model just for checking VIFs 
...
hist(primates$logECV)
hist(primates$logHRS)

#are there any confounds? 
hist(primates$logBodyMass) #larger body size = larger brain in nearly all animals 
hist(primates$foragingGS) #maybe the size of the foraging roup influences home range size.. more individuals = larger range 
...
plot(primates$logBodyMass, primates$logECV)
...
#any others to add? any links between them? any interactions? maybe body mass interacts with these things... since larger species might have smaller groups with larger home ranges?
#anything that affects some of the predictors but NOT directly effects brain size? 

p1 <- glmmTMB(logECV~ logHRS + logBodyMass + foragingGS + logHRS*foragingGS + logHRS*logBodyMass, data=primates)
summary(p1)
...
#dummy model without interactions in order to check VIFs
p2 <- glmmTMB(logECV ~ logHRS + logBodyMass + foragingGS, data=primates)
check_collinearity(p2)

summary(p1)

#marginal (total) effect of home range size 
estimate_slopes(p1, trend="logHRS")
estimate_slopes(p1, trend="logBodyMass")
estimate_slopes(p1, trend="foragingGS")


#interactions..
summary(primates$foragingGS)
estimate_slopes(p1, trend=c("logHRS"), by=list(foragingGS=c(1,10,20,50,100,200)))
#numbers can be hard to interpet, let's plot the interaction: 

#plotting a continous*continuous interaction 
p3 <- lm(logECV~ logHRS + logBodyMass + foragingGS + logHRS*foragingGS + logHRS*logBodyMass, data=primates)
summary(p3)
interplot(m=p3, var1="logHRS", var2 = "foragingGS") +
  labs(x = "Foraging Group Size", y = "Effect Home Range Size on ECV")

#navigating alone is harder than navigating together?! (have students brainstorm in their code and then share possible answers)
#how does foraging group size effect our main relationship of home range size on ECV? 
#only in very small groups is there a positive effect of home range on brain size! 
#cognitive offloading??!! 
#how do we interpret this with the overall effect being significant? 

hist(primates$foragingGS)

mean(primates$foragingGS, na.rm=TRUE)
